...
mycursor.execute("CREATE DATABASE IF NOT EXISTS mythiliphonepe;")
print("✅ Database 'mythiliphonepe' is ready.")

# Step 2: Switch to the correct database
mycursor.execute("USE mythiliphonepe;")
...
mycursor.execute("USE mythiliphonepe;")
print("✅ Using Database 'mythiliphonepe'.")
...
if case_study == "Decoding Transaction Dynamics":
    ...
    df = fetch_data("SELECT State, Year, Quarter, Insurance_type, Insurance_count FROM Aggregated_insurance")
    
    # Bar Chart
    fig = px.bar(df, x="State", y="Insurance_count", color="Insurance_type",
                 title="Insurance Engagement Across States")
    st.plotly_chart(fig)
    ...
    df = fetch_data("SELECT State, District, Year, Quarter, Transaction_count, Transaction_amount FROM Map_map")
    
    # Bar Chart
    fig = px.bar(df, x="State", y="Transaction_count", color="District",
                 title="Transaction Volume by State and District", barmode="stack")
    st.plotly_chart(fig)

elif case_study == "User Registration Analysis":
    st.header("User Registration Analysis")
    df = fetch_data("SELECT State, District, Year, Quarter, RegisteredUser FROM Map_user")
    
    # Heatmap
    pivot_df = df.pivot_table(values="RegisteredUser", index="State", columns="Year", aggfunc="sum")
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.heatmap(pivot_df, annot=True, cmap="viridis", fmt=".0f", linewidths=0.5)
    st.pyplot(fig)

elif case_study == "Insurance Transactions Analysis":
    st.header("Insurance Transactions Analysis")
    df = fetch_data("SELECT State, District, Year, Quarter, Insurance_count FROM Map_insurance")
    
    # Pie Chart
    fig = px.pie(df, values="Insurance_count", names="State",
                 title="Insurance Transactions Across States")
    st.plotly_chart(fig)
